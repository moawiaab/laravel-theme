import { CliOrders, Orders, BackOrders } from '@/types/columns';
import axios from "axios";
import type { SupplierOrder } from "@/types";
import { Notify } from 'quasar';
import { useTables } from '../tables';
import { defineStore } from 'pinia';

const route = "/orders";

export const useOrdersIndex = defineStore("orders-store", {
    state: () => ({
        entry: <SupplierOrder>{},
        lists: {
            roles: [],
        },
        loading: false,
        alert: false,
        confirm: false,
        backId: false,
        backOne: false,
        rowId: null,
        childId: null,
        errors: {
            name: "",
            num: ""
        },
    }),
    getters: {
        columns: () => Orders,
        supOrder: () => CliOrders,
        columnsBack: () => BackOrders
    },
    actions: {
        fetchCreateData() {
            axios.get(`${route}/create`).then((response) => {
                this.lists = response.data.meta;
            });
        },

        //start in edit
        fetchEditData(id: Number) {
            axios.get(`${route}/${id}/edit`).then((response) => {
                this.entry = response.data.data ?? [];
                this.lists = response.data.meta ?? [];
                // useTables().row = {}
            });
        },

        fetchShowData(id: number) {
            this.loading = true;
            axios
                .get(`${route}/${id}`)
                .then((response) => {
                    this.entry = response.data.data ?? [];
                    this.lists = response.data.meta ?? [];
                });
            this.loading = false
        },

        // send data to server in created
        storeData() {
            this.loading = true;
            return new Promise(async (resolve, reject) => {
                await axios
                    .post(route, this.entry)
                    .then((response) => {
                        Notify.create({
                            message: "تم إضافة فاتورة المبيعات بنجاح",
                            type: 'positive',

                        })
                        useTables().getData();
                        useTables().newRow = false
                        this.loading = false;
                        resolve(response);
                    })
                    .catch((error) => {
                        this.errors = error.response.data.errors || this.errors;
                        Notify.create({
                            message: error.response.data.message,
                            type: 'warning',

                        })
                        this.loading = false;
                        reject(error);
                    });
            });
        },

        // send data to server in updated
        updateData(id: number) {
            this.loading = true;
            return new Promise(async (resolve, reject) => {
                await axios
                    .put(`${route}/${id}`, this.entry)
                    .then((response) => {
                        Notify.create({
                            message: "تم تعديل بيانات فاتورة المبيعات بنجاح",
                            type: 'positive',

                        })
                        useTables().getData();
                        useTables().editRow = false
                        this.loading = false;
                        resolve(response);
                    })
                    .catch((error) => {
                        this.errors = error.response.data.errors || this.errors;
                        Notify.create({
                            message: error.response.data.message,
                            type: 'warning',

                        })
                        this.loading = false;
                        reject(error);
                    });
            });
        },
        setDone() {
            this.loading = true;
            axios
                .put(`${route}/${this.childId}/done`)
                .then((response) => {
                    useTables().getData();
                    // this.fetchShowData(1)
                    this.alert = false
                    this.childId = null;
                })
                .catch((error) => {
                    Notify.create({
                        message: error.response.data.message,
                        type: 'warning',
                    })
                })
                .finally(() => (this.loading = false));
        },

        delete() {
            axios
                .delete(`${route}/${this.rowId}`)
                .then((response) => {
                    Notify.create({
                        message: 'تم استلام جميع الاصناف بنجاح!',
                        type: 'positive',

                    })
                    useTables().getData();
                    this.rowId = null,
                        this.confirm = false;
                })
                .catch((error) => {
                    Notify.create({
                        message: error.response.data.message || "",
                        type: "warning"
                    })
                });
        },

        setBack() {
            this.loading = true;
            axios
                .put(`${route}/${this.childId}/back-all`)
                .then((response) => {
                    useTables().getData();
                    this.backId = false
                    this.childId = null;
                })
                .catch((error) => {
                    Notify.create({
                        message: error.response.data.message,
                        type: 'warning',
                    })
                })
                .finally(() => (this.loading = false));
        },

        setBackOne() {
            this.loading = true;
            axios
                .put(`${route}/${this.childId}/back`)
                .then((response) => {
                    useTables().getData();
                    this.backOne = false
                    this.childId = null;
                })
                .catch((error) => {
                    Notify.create({
                        message: error.response.data.message,
                        type: 'warning',
                    })
                })
                .finally(() => (this.loading = false));
        },
        doneBack() {
            this.loading = true;
            axios
                .put(`${route}/${this.rowId}/backDone`)
                .then((response) => {
                    useTables().getData();
                    this.confirm = false
                    this.rowId = null;
                })
                .catch((error) => {
                    Notify.create({
                        message: error.response.data.message,
                        type: 'warning',
                    })
                })
                .finally(() => (this.loading = false));
        }
    }
});
